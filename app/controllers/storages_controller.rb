class StoragesController < ApplicationController
  ...
  # GET /storages.json
  def index
    @storages = Storage.where("nvl(name,' ') like '%#{params[:name] || ''}%'")
                 .page(params[:page] || 1)
  end
  ...
  def printer_storage_label
    storage = Storage.where(:uuid => params[:uuid]).first

    printer = Printer.find params[:printer_id]
    socket  = TCPSocket.new(printer.ip, printer.port)

    hash        = {
        :name        => storage.werks.blank? ? '' : ''
    }
    zpl_command = Barcode.finish_storage_label hash
    socket.write zpl_command
    socket.close
    redirect_to print_storage_storages_url
  end

  def print_storage
    if params[:Storage].present?
      @storage = Storage.find_by_uuid(params[:storage])
      @storage = Storage.find_by_seq(params[:storage]) if @storage.blank?
    end
  end
